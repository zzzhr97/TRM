...
# ===== ADDED: reward extra metrics import =====
...
# ===== END ADDED: reward extra metrics import =====
from verl.trainer.ppo.mismatch_helper import compute_rollout_importance_weights
from verl.trainer.ppo.reward import compute_reward, compute_reward_async, RewardThread
...
class RayPPOTrainer:
    ...
    def __init__(
        ...
        self.reward_thread = RewardThread(config=self.config, tokenizer=self.tokenizer, reward_fn=self.reward_fn)
        #### Reward worker thread for threading-based reward computation ####
    ...
    def _validate(self):
        ...
        for test_data in self.val_dataloader:
            ...
            # ===== ensure rm_scores available during validation =====
            if self.use_rm and self.rm_wg is not None and "rm_scores" not in test_batch.batch.keys():
                reward_tensor_rm = self.rm_wg.compute_rm_score(test_batch)
                test_batch = test_batch.union(reward_tensor_rm)
            # ===== ensure rm_scores available during validation =====
    ...
    def fit(self):
        ...
        for epoch in range(self.config.trainer.total_epochs):
            for batch_dict in self.train_dataloader:
                ...
                # ===== ADDED: init reward_extra_infos_dict for logging =====
                reward_extra_infos_dict: dict[str, list] = {}
                # ===== END ADDED: init reward_extra_infos_dict for logging =====
                ...
                with marked_timer("reward", timing_raw, color="yellow"):
                    ...
                    if self.config.reward_model.launch_reward_fn_async:
                        ...
                        reward_req_id = self.reward_thread.submit(batch)
                    else:
                    ...
                    with marked_timer("adv", timing_raw, color="brown"):
                        ...
                        if self.config.reward_model.launch_reward_fn_async:
                            ...
                            reward_tensor, reward_extra_infos_dict = self.reward_thread.get(reward_req_id)
                        batch.batch["token_level_scores"] = reward_tensor

                        if reward_extra_infos_dict:
                            batch.non_tensor_batch.update({k: np.array(v) for k, v in reward_extra_infos_dict.items()})

                        # ===== ADDED: train-aux wandb metrics from reward_extra_info =====
                        timing_reward_keys = {
                            k
                            for k in reward_extra_infos_dict
                            if k.startswith("reward_timing/")
                        }
                        verify_keys = {
                            k
                            for k in reward_extra_infos_dict
                            if k.startswith("verify_") and k.removeprefix("verify_").isdigit()
                        }
                        for k in timing_reward_keys:
                            ...
                            metrics[f"train-aux-reward-timing_s/{suffix}"] = val
                        metrics.update(
                            compute_reward_extra_metrics(
                                reward_extra_infos_dict,
                                prefix="train-aux",
                                skip_keys={"verify_meta", *verify_keys, *timing_reward_keys},
                            )
                        )
                        if verify_keys:
                            for k in verify_keys:
                                ...
                                if isinstance(vals, (list, tuple)) and vals:
                                    ...
                                    try:
                                        ...
                                        continue
                                metrics[f"train-aux-rollout/{k}"] = val
                        # ===== END ADDED: train-aux wandb metrics from reward_extra_info =====
