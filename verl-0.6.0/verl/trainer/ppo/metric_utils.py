...
# ===== ADDED: reward extra metrics aggregation =====
def compute_reward_extra_metrics(
    reward_extra_infos_dict: dict[str, list[Any]],
    prefix: str = "train-aux",
    stats: tuple[str, ...] = ("mean", "min", "max"),
    skip_keys: set[str] | None = None,
) -> dict[str, float]:
    """
    Aggregate reward_extra_info lists into scalar metrics for logging.

    - Ignores empty entries and non-numeric data.
    - Skips keys in skip_keys (e.g., JSON strings like verify_meta).
    - Uses NaN-safe reductions.
    """
    ...
    for key, vals in reward_extra_infos_dict.items():
        ...
        # Attempt to coerce object arrays to float; skip if not numeric
        if arr.dtype == object:
            try:
        ...
        for stat in stats:
            if stat == "mean":
                ...
                continue
            metrics[f"{prefix}/{key}/{stat}"] = val

    return metrics


# ===== END ADDED: reward extra metrics aggregation =====
