    ...
    try:
        if "\pi" in extracted_model_output or "\pi" in ground_truth:
            ...
            for pi in [math.pi, 3.14]:
                equivs.append(math_equal(extracted_model_output, ground_truth, timeout=timeout, pi=pi))
            is_correct = any(equivs)
        else:
            is_correct = math_equal(extracted_model_output, ground_truth, timeout=timeout)
    except Exception:
        is_correct = False
...
if __name__ == "__main__":
    ...
    gt = "(3, \\frac{\\pi}{2})"
    score, format_correctness, extracted_answer = compute_score(pred, gt)
    print(f"Score: {score}, Format Correctness: {format_correctness}, Extracted Answer: {extracted_answer}")