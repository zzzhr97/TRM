...
def is_equivalent(actual_raw, expect_raw):
    ...
    def _safe_float_from_str(text: str, max_int_digits=16):
        if not isinstance(text, str):
            return None
        text = text.strip()
        if "." not in text:
            return None
        text_unquoted = _dequote(text)
        mantissa = text_unquoted
        if "e" in mantissa.lower():
            mantissa = mantissa.lower().split("e", 1)[0]
        if "." not in mantissa:
            return None
        int_part = mantissa.split(".", 1)[0]
        int_digits = int_part.lstrip("+-")
        if len(int_digits) > max_int_digits:
            return None
        if not _re.match(r"^[+-]?(?:\d*\.\d+|\d+\.\d*)(?:[eE][+-]?\d+)?$", text_unquoted):
            return None
        try:
            value = float(text_unquoted)
        except Exception:
            return None
    ...
    def _safe_float_from_val(value):
        if isinstance(value, float):
            return value if _math.isfinite(value) else None
        if isinstance(value, str):
            return _safe_float_from_str(value)
        return None

    def _floats_close(a_value, b_value, abs_tol=1e-9):
        if a_value is None or b_value is None:
            return False
        return abs(a_value - b_value) <= abs_tol

    def _lists_close_floatlike(list_a, list_b):
        if not (isinstance(list_a, (list, tuple)) and isinstance(list_b, (list, tuple))):
            return False
        if len(list_a) != len(list_b):
            return False
        for val_a, val_b in zip(list_a, list_b):
            if not _floats_close(_safe_float_from_val(val_a), _safe_float_from_val(val_b)):
                return False
        return True

    actual = _normalize_for_compare(actual_raw)
    expect = _normalize_for_compare(expect_raw)

    if actual == expect:
        return True

    if _dequote(actual) == expect or actual == _dequote(expect) or _dequote(actual) == _dequote(expect):
        return True

    if _normalize_ws(actual) == _normalize_ws(expect):
        return True

    actual_json, expect_json = _try_json(actual), _try_json(expect)
    if actual_json is not None and expect_json is not None:
        if actual_json == expect_json:
            return True
        if _floats_close(_safe_float_from_val(actual_json), _safe_float_from_val(expect_json)):
            return True
        if _lists_close_floatlike(actual_json, expect_json):
            return True

    if (
        actual_json is not None
        and isinstance(actual_json, (str, float, bool))
        and _normalize_for_compare(str(actual_json)) == expect
    ):
        return True
    if (
        expect_json is not None
        and isinstance(expect_json, (str, float, bool))
        and _normalize_for_compare(str(expect_json)) == actual
    ):
        return True

    if actual_json is not None and isinstance(actual_json, (str, float)):
        if _floats_close(_safe_float_from_val(actual_json), _safe_float_from_val(expect)):
            return True
    if expect_json is not None and isinstance(expect_json, (str, float)):
        if _floats_close(_safe_float_from_val(expect_json), _safe_float_from_val(actual)):
            return True
    ...
    if error_msg:
        ...
        if api_status == "SandboxError":
            ...
            if run_result and metadata["run_status"] == "Finished":
                ...
                if expected_output is None or is_equivalent(actual_output, expected_output):
                    result_status = True
