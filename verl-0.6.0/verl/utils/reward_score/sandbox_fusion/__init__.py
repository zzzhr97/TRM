...
def compute_score(
    ...
    """
    solution = completion
    # --- patched: ensure we always grab the *last* code fence ---
    if "```python" in completion:
        ...
        if len(parts) >= 2:
            # Locate the last fenced code block (odd indices after split)
            content_idx = None
            for idx in range(len(parts) - 1, 0, -1):
                if idx % 2 == 1 and parts[idx].strip():
                    content_idx = idx
                    break
            if content_idx is None:
                content_idx = 1
            solution = parts[content_idx]
    ...
    try:
        ...
        if continuous:
            # Calculate pass rate for all available test cases (previously truncated to 10)
            num_to_consider = len(res_list)
