...
# ===== ADDED: torch.save retry helper =====
def _torch_save_with_retry(obj, path, max_retries: int = 32) -> None:
    """
    Save with retry on failure. Delay schedule: 1,2,4,8,16,32,32,...
    ...
    for attempt in range(max_retries + 1):
        try:
            ...
            logger.warning(
                "torch.save failed for %s (attempt %d/%d): %s; retrying in %ss",
                path,
                attempt + 1,
                max_retries + 1,
                exc,
            ...
            time.sleep(delay)
# ===== END ADDED: torch.save retry helper =====
        ...
        with warnings.catch_warnings():
            ...
            with get_fsdp_state_ctx(self.model, StateDictType.SHARDED_STATE_DICT, state_dict_cfg, optim_cfg):
                ...
                if self.should_save_model:
                    ...
                    # ===== ADDED: torch.save retry =====
                    _torch_save_with_retry(model_state_dict, model_path)
                    # ===== END ADDED: torch.save retry =====
                    log_with_rank(f"Saved model to {os.path.abspath(model_path)}", rank=self.rank, logger=logger)

                if self.should_save_optimizer:
                    optimizer_state_dict = self.optimizer.state_dict()
                    # ===== ADDED: torch.save retry =====
                    _torch_save_with_retry(optimizer_state_dict, optim_path)
                    # ===== END ADDED: torch.save retry =====
                ...
                if self.should_save_extra:
                    ...
                    # ===== ADDED: torch.save retry =====
                    _torch_save_with_retry(extra_state_dict, extra_path)
                    # ===== END ADDED: torch.save retry =====
